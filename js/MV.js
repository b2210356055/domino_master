...
//----------------------------------------------------------------------------
function mult_matrix(u, v){
    if(u.length !== v.length){
        console.error("Array dimensions are not matching!!\nu.length= ", u.length, "\nv.length= ", v.length);
        return -1;
    }
    let n = Math.sqrt(u.length);
    if(!Number.isInteger(n)){
        console.error("Array length(",n,") doesn't represent a square matrix!!!");
        return -1;
    }
    let result = new Array(n * n).fill(0); // Initialize the result matrix (1D array)
    ...
    for (let i = 0; i < n; i++) {
        for (let j = 0; j < n; j++) {
            let sum = 0;
            for (let k = 0; k < n; k++) {
                sum += u[i * n + k] * v[k * n + j]; // Dot product
            }
            result[i * n + j] = sum; // Store result in 1D array format
        }
    }
    return result;
}
